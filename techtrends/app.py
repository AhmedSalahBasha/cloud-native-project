...
def get_db_connection():
    ...
    try:
        ...
        logger.error("Error {0} has occured while connecting to database.db!".format(err))
...
def get_post(post_id):
    ...
    post = connection.execute('SELECT * FROM posts WHERE id = ?',
                        (post_id,)).fetchone()
    connection.close()
    logger.info("Post ID {0} is retrieved!".format(post_id))
    return post
...
def post(post_id):
    ...
    if post is None:
      logger.warning("Post not found in the database!")
      return render_template('404.html'), 404
    else:
      logger.info("Post {0} was found in the database".format(post[2]))
